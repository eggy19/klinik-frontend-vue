...
export const useMedicineStore = defineStore('medicine', () => {
  ...
  const lowStockCount = computed(() => items.value.filter((m) => m.stock <= m.minStock).length)
  ...
  async function fetchAll() {
    loading.value = true
    try {
      items.value = await medicineApi.getMedicines()
    } finally {
      loading.value = false
    }
  }
  ...
  async function create(input: MedicineInput) {
    const created = await medicineApi.createMedicine(input)
    items.value = [created, ...items.value]
    return created
  }

  async function update(id: string, input: MedicineInput) {
    const updated = await medicineApi.updateMedicine(id, input)
